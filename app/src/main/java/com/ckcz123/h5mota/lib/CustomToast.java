...
public class CustomToast {
    ...
    private static void showToast(Context context,
        ...
        if (type == TYPE_SUCCESS) {
            ViewSetting.setImageResource(view, R.id.toast_image, R.mipmap.success);
            view.setBackgroundColor(Color.parseColor("#b4eeb4"));
        } else if (type == TYPE_INFO) {
            ViewSetting.setImageResource(view, R.id.toast_image, R.mipmap.info);
            view.setBackgroundColor(Color.parseColor("#87ceeb"));
        } else if (type == TYPE_ERROR) {
            ViewSetting.setImageResource(view, R.id.toast_image, R.mipmap.error);
