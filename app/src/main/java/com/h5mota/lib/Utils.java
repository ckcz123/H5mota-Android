...
public class Utils {
  ...
  public static boolean unzip(File file, File directory) {
    ...
    try (InputStream inputStream = new FileInputStream(file);
        ZipInputStream zipInputStream =
            new ZipInputStream(new BufferedInputStream(inputStream), Charset.forName("GBK"))) {
  ...
  public static Bitmap captureWebView(WebView webView) {
    ...
    if (bg != null) canvas.drawBitmap(bg, 0, 0, new Paint());
  ...
  public static String generateColorString() {
    ...
    int[] x = new int[6];
    while (true) {
      for (int i = 0; i < 6; i++) x[i] = random.nextInt(16);
      int val = x[0] + x[2] + x[4];
      if (val >= 27 && val <= 40) break;
    }
    String string = "#";
    for (int i = 0; i < 6; i++) string += intToHex(x[i]);
    return string;
  }
  ...
  public static String getValue(String url, String name) {
    ...
    if (pos < 0) return "";
    String string;
    if (anchor <= pos + 1) string = url.substring(pos + 1);
    else string = url.substring(pos + 1, anchor - pos - 1);
    if ("".equals(string)) return "";
    String[] strings = string.split("&");
    for (String str : strings) {
      String[] para = str.split("=");
      if (name.equals(para[0].trim())) return para[1].trim();
    }
    return "";
  }
}
