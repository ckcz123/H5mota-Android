...
public class GifImageView extends AppCompatImageView implements Runnable {
  ...
  private final Runnable updateResults =
      new Runnable() {
        @Override
        public void run() {
          if (tmpBitmap != null && !tmpBitmap.isRecycled()) setImageBitmap(tmpBitmap);
        }
      };
    ...
    do {
      for (int i = 0; i < n; i++) {
        ...
        try {
          ...
          if (frameCallback != null) tmpBitmap = frameCallback.onFrameAvailable(tmpBitmap);
...
class GifDecoder {
  ...
  public void advance() {
    if (frameCount > 0) framePointer = (framePointer + 1) % frameCount;
  }
